...
def month_from_name(month_name):
    try:
        ...
        assert False, invalid_month_name(month_name)

def invalid_month_name(month_name):
...
El URL se encuentra en el atributo =data-duration-seconds= de un tag =<button>= que tiene la clase =replay-button=. 
