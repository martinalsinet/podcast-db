...
1. Un div que tiene el ~<a>~ con la clase correcta y el atributo =title=
2. Un div que no tiene el tag ~<a>~
3. Un div que tiene el tag ~<a>~ pero éste no la clase =rich-section...=
4. Un div que tiene el tag ~<a>~ pero éste no tiene el atributo =title=
...
3. El html no contiene un tag ~<a>~ devuelve un string vacío
4. El html tiene el tag ~<a>~ y pero éste no tiene la clase correcta, devuelve un string vacío
5. El html tiene el tag ~<a>~ y pero éste no tiene el atributo =title=, devuelve un string vacío
...
2. El parámetro tag no es un bs4.element.tag, lanza un =AssertionError=
3. El html no tiene un tag ~<a>~, devuelve un string vacío
4. El html tiene el tag ~<a>~ pero éste no tiene la clase buscada, devuelve un string vacío
5. El html tiene el tag ~<a>~ pero éste no tiene el atributo =href=, devuelve un string vacío
...
- url :: es un =string=, debe ser un URL válido y también es obligatorio
...
Vamos a generar los diferentes ejemplos de tags que nuestra función puede llegar a recibir:

- Un tag que tenga todos los elementos para generar un podcast válido (título, URL y autor)
- Un tag al que le falte el elemento del título
- Un tag que le falte el elemento del URL
- Un tag que tenga el elemento del URL, pero que éste no sea válido
- Un tag que no tenga autor
- Un tag que no sea un =bs4.element.Tag=
...
def podcast_from_tag(tag):
    ...
    # extraer el autor con author_from_tag
...
2. El tag no tiene título, lanza un =AssertionError=
3. El tag no tiene url, lanza un =AssertionError=
4. El tag tiene un url pero es inválido, lanza un =AssertionError=
5. El tag no tiene autor, devuelve un podcast con un =string= vacío en la propiedad author
6. El tag no es un =bs4.element.Tag=, lanza un =AssertionError=
...
1. Un html que no tenga ningún ~<div>~
2. Un html que tenga varios ~<div>~ pero que ninguno tenga la clase =rich-section-list-item-content=
3. Un html que tenga algunos ~<div>~ con la clase buscada y otros que no la tienen
4. Un html que tenga varios ~<div>~ con la clase buscada
5. Un html que no sea un =string=
...
1. Cuando recibe un html que no tenga ningún ~<div>~, devuelve una lista vacía
2. Cuando recibe un html que tenga varios ~<div>~ pero ninguno tiene la clase =rich-section-list-item-content=, devuelve una lista vacía
3. Cuando recibe un html que tiene algunos ~<div>~ con la clase buscada y otros que no la tienen, devuelve una lista con solamente los ~<div>~ que tienen la clase
4. Cuando recibe un html que tiene varios ~<div>~ con la clase buscada, devuelve una lista de tantos elementos como ~<div>~ hay en el html
5. Cuando recibe un html que no es un string, lanza un =AssertionError=
...
1. Una lista vacía
2. Una lista con objetos que no son del tipo =bs4.element.Tag=
3. Una lista con un número exacto de tags

#+begin_src python :tangle franceinter/podcasts/tests/examples_podcasts_from_tags.py
from franceinter.podcasts.tags_from_html import tags_from_html
from franceinter.podcasts.tests.examples_tags_from_html import *
from franceinter.podcasts.tests.examples_podcast_from_tag import tag_ok
...
1. Si recibe una lista vacía, devuelve una lista vacía
2. Si recibe una lista con objetos que no son del tipo =bs4.element.Tag=, lanza un =AssertionError=
3. Si recibe una lista de tags, devuelve una lista de objetos =franceinter.podcasts.Podcast=
